...
export function ParenthesesElement({ element, isSelected }: Props) {
  const style = element.style ?? {}
  const text = element.dataKey ?? element.content
  const display = text != null && text !== '-' ? ` ${text} ` : '-'

  return (
    <div
      className={`element-text element-parentheses ${isSelected ? 'selected' : ''}`}
    ...
    >
      {element.gaseousKey ? `[${element.gaseousKey}]` : display}
    </div>
  )
}
